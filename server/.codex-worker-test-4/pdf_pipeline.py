...
class Renderer:
    ...
    def extract_text(
        ...
        return []  # pragma: no cover - optional interface
...
class PdfiumRenderer(Renderer):
    ...
    def extract_text(
        ...
        """Extract positioned lines from PDFium's embedded character layer."""
        ...
        def flush() -> None:
            ...
            if text and boxes:
                x0 = max(0.0, min(box[0] for box in boxes) * rendered_page.scale)
                y0 = max(
                    0.0,
                    rendered_page.height
                    - max(box[3] for box in boxes) * rendered_page.scale,
                )
                x1 = min(
                    float(rendered_page.width),
                    max(box[2] for box in boxes) * rendered_page.scale,
                )
                y1 = min(
                    float(rendered_page.height),
                    rendered_page.height
                    - min(box[1] for box in boxes) * rendered_page.scale,
                )
                if x1 > x0 and y1 > y0:
                    lines.append(OcrLine(text=text, confidence=1.0, bbox=(x0, y0, x1, y1)))
            characters.clear()
            boxes.clear()
        ...
        try:
            ...
            for index in range(text_page.count_chars()):
                ...
                except Exception:  # noqa: BLE001 - malformed char box lowers quality
                    box = None
                if box is not None:
                    if boxes:
                        previous = boxes[-1]
                        previous_center = (previous[1] + previous[3]) / 2.0
                        current_center = (box[1] + box[3]) / 2.0
                        height = max(1.0, previous[3] - previous[1], box[3] - box[1])
                        if abs(current_center - previous_center) > height * 1.5:
                            flush()
    ...
    def process_file(
        ...
        try:
            ...
            for page_number in range(page_start, last + 1):
                ...
                lines: list[OcrLine] = []
                quality = assess_text_layer(lines)
                extract_text = getattr(self._renderer, "extract_text", None)
                if callable(extract_text):
                    try:
                        lines = extract_text(document, page_number - 1, page)
                        quality = assess_text_layer(lines)
                    except Exception:  # noqa: BLE001 - bad text layers fall back safely
                        lines = []
                        quality = assess_text_layer(lines)
                        logger.warning(
                            "page %s embedded text extraction failed; fallback=visual_ocr",
                            page_number,
                        )
                try:
                    if quality.reliable:
                        logger.info(
                            "page %s embedded text quality=%s score=%.2f; source=embedded_text",
                            page_number,
                            quality.reason,
                            quality.score,
                        )
                        ...
                        logger.info(
                            "page %s embedded text quality=%s score=%.2f; fallback=visual_ocr",
                            page_number,
                            quality.reason,
                            quality.score,
                        )
